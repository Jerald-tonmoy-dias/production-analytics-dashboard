...
The Query provider is mounted on the **orders segment layout**, not the root layout, so the Dashboard does not pay for a client provider it does not use. `ThemeProvider` (`next-themes`, class strategy, `defaultTheme="light"`) is a leaf around `{children}` in the root layout so `.dark` is available everywhere without marking that layout `"use client"`.
